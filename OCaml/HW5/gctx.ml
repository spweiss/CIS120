...
(* Adds a thickness parameter to the type definition of gctx *)
...
(* Incorporates thickness into the top_level definition *)
...
let with_thickness (g: gctx) (t: int) : gctx =
  { g with thickness = t }
(* Adds a with_thickness function to output a gctx with a given thickness*)
(* as default *)
...
(* Updates set_graphics_state to also update the thickness *)
...
let draw_rect (g: gctx) (p1: position) ((w, h): dimension) : unit =
  (* Translates the coordinates to OCaml coordinates, moves to the correct*)
  (* location, depending on the height of the rectangle, and draws the*)
  (* rectangle of the specified dimensions*)
  set_graphics_state g;
...
let draw_ellipse (g: gctx) (p: position) (rx: int) (ry: int) : unit =
  (* Translates the coordinates to OCaml coordinates, moves to the correct*)
  (* location, and draws the ellipse of the specified radii*)
  set_graphics_state g;
  let (x, y) = ocaml_coords g p in
  Graphics.moveto x y;
...
let draw_point (g: gctx) (p: position) : unit =
  ...
  let (x, y) = ocaml_coords g p in
  Graphics.plot x y
  (* Plots a point based on a given postion *)

let draw_points (g: gctx) (ps: position list) : unit =
  set_graphics_state g;
  List.iter (draw_point g) ps
  (* Plots a list of points based on their positions *)
